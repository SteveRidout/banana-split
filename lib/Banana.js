...
module.exports = function (config) {
		...
		async.waterfall([
			...
			function (experiment, participants, callback) {
				models.result.findOneAndUpdate({
					experiment: experiment.name,
					event: eventName
				}, config.unitTest ? {$set: {__v: 0}} /* workaround for mockgoose problem */ : {}, {
					upsert: true
				}, function (err, result) {
					// TODO: decide whether to recalculate
					...
					callback(err, result, participants);
				});
			},
			function (result, participants, callback) {
				var variations = {};
	...
	publicAPI.optOut = function (options, callback) {
		// Deletes all events and participations of the user in question
		// Used to remove events from pre-logged-in users
		async.waterfall([
			function (cb) {
			...
			function (cb) {
				// remove all events generated by this user
				// TODO: should we keep these and just mark as potentially useful?
				models.event.remove({user: options.user}, function (err) {
					cb(err);
				});
			}
		], function (err) {
