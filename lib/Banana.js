...
module.exports = function (config) {
	...
	var Participation = ParticipationFactory(config);
	var Experiment = ExperimentFactory(config);
			...
			function (experiment, participant, callback) {
				...
				if (!variation) {
					if (options.variation) {
						if (!_.contains(experiment.variations, options.variation)) {
							callback("Variation not valid: " + options.variaiton);
							return;
						}
						...
						variation = experiment.variations[
							parseInt(md5(options.experiment + options.participant).substring(0, 8), 16) %
	...
	publicAPI.optOut = function (options, callback) {
		...
		if (options.ifNotConverted) {
			query.converted = false;
		}
		
		Participation.findOneAndUpdate(query, {$set: {optedOut: true}}, function (err, removed) {
			callback(err);
		});
	};
